...
try:
	...
	for i in range(STUDENTS):
		...
		cur_proficiency = generate_proficiency([0.5,0.45,0.05])
		df = df.append(other = build_row(cur_id, "Prior Assessment Test", 90, cur_proficiency), ignore_index = True)
		while True:
			terminate_probability = (sum(cur_proficiency) - 6.0)/(NUM_KC*(NUM_PROFICIENCY_LEVEL-1.0))
			...
			if step < 10:
				if action_type[:2] == "AT":
					reveal_plevels = all_actions["reveal_plevels"][action_type]
					reveal_proficiency = {}
					for i in range(NUM_KC):
	...
	print "no actions generated yet!"
