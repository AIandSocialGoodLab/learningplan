...
def generate_proficiency(single_maximum_proficiency):
	p_level = []
	for i in range(NUM_KC):
		p_level.append(random.randint(0,single_maximum_proficiency))
	return p_level
...
for i in range(STUDENTS):
	...
	cur_proficiency = generate_proficiency(2)
	df = df.append(other = build_row(cur_id, "Prior Assessment Test", 90, cur_proficiency), ignore_index = True)
	continue_do_action = True
	while continue_do_action:
		terminate_probability = (sum(cur_proficiency) - 10.0)/15.0
		if random.random() < terminate_probability:
			continue_do_action = False
		...
		for state in all_actions[action_type]:
			...
			if state_match(cur_state, cur_proficiency):
				...
				break
		if action_type == "take assessment test":
		...
		if sum(cur_proficiency) == NUM_KC*(NUM_PROFICIENCY_LEVEL -1):
			continue_do_action = False
	df = df.append(other = build_row(cur_id, "Final Exam", 90, cur_proficiency), ignore_index = True)
	df = df.append(other = build_row(None, None, None, None), ignore_index = True)
